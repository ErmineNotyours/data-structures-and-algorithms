...
namespace llkthfromend
{
    public class SinglyLinked
    {
        ...
        /// <summary>
        /// Finds the kth item from the end of the list
        /// Traversal adapted from https://stackoverflow.com/questions/3823848/creating-a-very-simple-linked-list
        /// <para>k is the number of places from the end we are searching</para>
        /// </summary>
        public Node kthFromEnd(int k)
        {
            ...
            try
            {
                int length = 0; // used for counting through the list.  Will return the total length
                                // traverse the node
                while (cur.Next != null)
                {
                    //Console.WriteLine($"cur.Value: {cur.Value}");
                ...
                length++; // Now length equals the total number of list items
                //Console.WriteLine($"cur.Value: {cur.Value}"); // Another call for cur.Value because the loop kicks out early
                cur = Head; // Reset cur for retraversing
            ...
            return cur; // I don't really want to return a node on an exception, but the program won't compile unless I do.
